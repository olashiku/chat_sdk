package com.ghabie.chatsdkandroid.utils
...
fun RecyclerView.updateRecycler(
    context: Context,
...
fun RecyclerView.updateChatRecycler(
    ...
    this.layoutManager = LinearLayoutManager(context)
    (this.layoutManager as LinearLayoutManager).setStackFromEnd(true)
    val reyclerAdaptor = RecyclerAdapterUtil(context, listOfItems, layout)
    reyclerAdaptor.addViewsList(listOfLayout)
    reyclerAdaptor.addOnDataBindListener { itemView, item, position, innerViews ->
        binder(innerViews, position)
...
fun RecyclerView.updateRecyclers(
    context: Context,
    listOfItems: List<Any>,
    layout: Int,
    listOfLayout: List<Int>,
    binder: (Map<Int, View>, Int) -> Unit,
    onClickPosition: (Any, Int) -> Unit
): RecyclerView.Adapter<*>? {
    this.layoutManager = LinearLayoutManager(context)
    val reyclerAdaptor = RecyclerAdapterUtil(context, listOfItems, layout)
    reyclerAdaptor.addViewsList(listOfLayout)
    ...
    reyclerAdaptor.addOnClickListener { item, position -> onClickPosition(item, position) }
...
fun ViewPager.setUpViewPager(
    viewPagerObjectList: List<ViewPagerObject>,
    fragmentStateManager: FragmentManager
) {
